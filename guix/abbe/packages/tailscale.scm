...
(define (tailscale-arch (system (%current-system)))
  (match system
    ("aarch64-linux" "arm64")
    ("x86_64-linux" "amd64")
    ("i686-linux" "386")))
...
(define (tailscale-hash (system (%current-system)))
  (match system
...
(define-public tailscale
  (package
   ...
   (source (origin
            ...
            (uri (tailscale-url version)
                 (sha256 (base32 tailscale-hash))))
           (build-system copy-build-system)
           (arguments
            (list
             #:install-plan
             #~`((,(string-append "tailscale_" #$version "_" (tailscale-arch) "/") "bin/"))))
           (propagated-inputs
